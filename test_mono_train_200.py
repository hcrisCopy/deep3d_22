...
def get_args():
    ...
    parser.add_argument("--data_dir", default='/root/autodl-tmp/Data/mono_train', type=str, help="test dataset directory")
    parser.add_argument("--out_dir", default='/root/autodl-tmp/Data/deep3d_outputs_train200', type=str, help="output directory for predicted right views")
...
def evaluate():
    ...
    print(f"\nEvaluating on mono_train (first 10250)")
    ...
    if not os.path.exists(left_dir) or not os.path.exists(right_dir):
        print(f"Missing left or right folder in {args.data_dir}.")
        return
        
    filenames = sorted(os.listdir(left_dir))[:1000]
    
    for fname in tqdm(filenames, desc="mono_train_200"):
        # Read images
        ...
        if total_samples == 0:
            ...
            print(f"2. 模拟输入大小: Left = {left_img_1280.shape}, Right GT = {right_img_1280.shape}")
        ...
        # Save the predicted right view
        os.makedirs(args.out_dir, exist_ok=True)
        cv2.imwrite(os.path.join(args.out_dir, fname), pred_right_img_1280)
